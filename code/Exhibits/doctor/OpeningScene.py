...
class OpeningScene(Scene):
	def __init__(self, game):
		...
		self.startButton = Button(self.screen, pygame.Rect(self.screen.get_width() // 2 - 245 // 2, 490, 245, 78), 
	...
	def createTexts(self):
		self.headerText = self.smallScreenHeaderFont.render(self.config.getText("DFAM_OPENING_SCREEN_HEADER"), True, (255, 255, 255))
		self.subHeaderTexts = Utilities.renderTextList(self.config, self.smallTextFont, "DFAM_OPENING_SCREEN_SUB_HEADER")

	def draw(self, dt):
		self.screen.blit(self.background, (0, 0))
		Utilities.drawTextOnCenterX(self.screen, self.headerText, (self.screen.get_width() // 2, 257))
		Utilities.drawTextsOnCenterX(self.screen, self.subHeaderTexts, (self.screen.get_width() // 2, 351), 30)
		super().draw(dt)